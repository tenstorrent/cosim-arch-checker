 ...
 *                     Chronologic VCS (TM)
 *
 * SystemVerilog 1800 DPI (Direct Programming Interface).
 *
 * Binary-level compatibility include file "svdpi.h"
 *
 * Revision:
 *  as in Annex G of IEEE-Std-1800-SystemVerilog_LRM_2005
 *  Accellera's Extensions to Verilog (R)
 *
 *
 * This file contains the constant definitions, structure definitions, and
 * function declarations used by the SystemVerilog 1800 DPI.
 *
...
#ifndef VPI_VECVAL
#define VPI_VECVAL
/* For backward compatibility of IEEE STD 1800-2005 */
struct vpi_vecval {
    uint32_t aval;
    uint32_t bval;
};
...
/* typedef struct { unsigned int c; unsigned int d;} svLogicVecVal;  temp solution before Phase 2  */
...
XXTERN typedef void* svDynamicArrayHandle;
XXTERN svDynamicArrayHandle svAllocDynArray(const svDynamicArrayHandle h, int numElements);
XXTERN void* svGetDynArrayPtr(const svDynamicArrayHandle h);
XXTERN void* svGetDynArrayElemType(const svDynamicArrayHandle h);
XXTERN int svGetDynArrayElemNum(const svDynamicArrayHandle h);
XXTERN int svGetDynArrayElemSize(const svDynamicArrayHandle h);
